...
public class MybatisPageableInterceptor implements Interceptor {
	...
	private static Logger logger = LoggerFactory
			.getLogger(MybatisPageableInterceptor.class);
	...
	private Pageable findPageableObject(Object params) {
		...
		// 多个参数 表现为 ParamMap
			...
			for (Map.Entry<String, Object> entry : paramMap.entrySet()) {
				...
				if (paramValue != null
						&& Pageable.class.isAssignableFrom(paramValue
								.getClass())) {
		...
		try {
			...
			if (rs != null) {
				try {
					...
					logger.error(
							"exception happens when doing: ResultSet.close()",
							e);
				}
			}
			...
			if (countStmt != null) {
				try {
					...
					logger.error(
							"exception happens when doing: PreparedStatement.close()",
							e);
				}
			}
			...
			if (connection != null) {
				try {
					...
					logger.error(
							"exception happens when doing: Connection.close()",
							e);
				}
			}
		}
